...
function nextQuestion() {
    ...
    while (options.length < 5) {
        const c = flags[Math.floor(Math.random() * flags.length)].country;
        ...
        button.onclick = () => {
            ...
            buttons.forEach(btn => {
                if (btn.innerText === correctLabel) {
                    btn.style.background = "#244d2e";
                    btn.style.borderColor = "#42a05a";
                    btn.style.color = "#a0f0b0";
                } else {
                    btn.style.background = "#4d1c1c";
